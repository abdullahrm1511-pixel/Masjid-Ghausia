...
export default async function SurveysPage() {
  const surveys = await prisma.survey.findMany({ where: { templateKey: { in: ["DONOR_JOURNEY", "ONE_TIME_DONATION"] } }, include: { _count: { select: { responses: true } } }, orderBy: [{ templateKey: "asc" }, { createdAt: "desc" }] });
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-sm font-bold text-[#0f766e]">Instellingen</p><h1 className="text-3xl font-bold text-slate-900">Donatieformulieren</h1><p className="mt-2 text-slate-700">Beheer de vaste lidmaatschapsenquête en maak eenmalige donatiecampagnes.</p></div>
        <Link className="rounded-md bg-[#1483d6] px-4 py-3 font-semibold text-white" href="/admin/settings/surveys/new">Eenmalige donatie maken</Link>
      </div>
