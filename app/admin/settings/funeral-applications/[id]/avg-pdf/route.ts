...
import { avgFuneralApplicationFilename, generateAvgFuneralApplicationPdf } from "@/lib/pdf/avg-funeral-application";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pdf = await generateAvgFuneralApplicationPdf(data);
  return new NextResponse(new Uint8Array(pdf), { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${avgFuneralApplicationFilename(data)}"`, "Cache-Control": "private, no-store" } });
}
