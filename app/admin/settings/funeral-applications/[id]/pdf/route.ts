...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pdf = await generateFuneralApplicationPdf(data, application.submittedAt ?? new Date());
  return new NextResponse(new Uint8Array(pdf), { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${funeralApplicationFilename(data)}"`, "Cache-Control": "private, no-store" } });
}
