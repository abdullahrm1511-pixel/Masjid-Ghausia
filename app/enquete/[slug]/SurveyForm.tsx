...
export function SurveyForm({ survey, settings, preview = false }: { survey: { id: string; title: string; description: string | null; templateKey: string }; settings: FixedSurveySettings; preview?: boolean }) {
  ...
  if (state.step === "VERIFY_EXISTING") return <form action={action} className="survey-card"><input name="surveyId" type="hidden" value={survey.id} /><input name="mode" type="hidden" value="VERIFY_EXISTING" /><input name="challengeId" type="hidden" value={state.challengeId ?? ""} /><header className="survey-heading"><p className="donor-eyebrow">Beveiligde controle</p><h1>Bevestig dat u het bent</h1><p>{state.message}{state.maskedEmail ? ` Code verstuurd naar ${state.maskedEmail}.` : ""}</p></header><label>Verificatiecode<input autoComplete="one-time-code" inputMode="numeric" maxLength={6} name="verificationCode" pattern="[0-9]{6}" required /></label><button className="donor-submit-button survey-submit" disabled={pending} type="submit">{pending ? "Code controleren..." : "Code controleren"}</button><div className="text-center"><p className="text-sm text-slate-600">Geen code ontvangen? Controleer ook uw spammap.</p><button className="mt-2 font-bold text-[#0f5f9f] underline disabled:opacity-60" disabled={pending} formNoValidate name="intent" type="submit" value="RESEND_CODE">{pending ? "Even geduld..." : "Stuur de code opnieuw"}</button></div></form>;
  if (state.step === "EXISTING_OPTIONS") return <form action={action} className="survey-card"><input name="surveyId" type="hidden" value={survey.id} /><input name="mode" type="hidden" value="MEMBER_REQUEST" /><input name="challengeId" type="hidden" value={state.challengeId ?? ""} /><input name="accessToken" type="hidden" value={state.accessToken ?? ""} /><header className="survey-heading"><p className="donor-eyebrow">Bestaande donateur herkend</p><h1>Wat wilt u doen?</h1><p>{state.message}</p></header><div className="survey-choices"><label className="survey-choice"><input checked={memberAction === "CONFIRM"} name="memberAction" onChange={() => setMemberAction("CONFIRM")} type="radio" value="CONFIRM" /><span>Mijn donateurschap bevestigen</span></label><label className="survey-choice"><input checked={memberAction === "INCREASE"} name="memberAction" onChange={() => setMemberAction("INCREASE")} type="radio" value="INCREASE" /><span>Mijn maandelijkse donatie verhogen</span></label><label className="survey-choice"><input checked={memberAction === "CANCEL"} name="memberAction" onChange={() => setMemberAction("CANCEL")} type="radio" value="CANCEL" /><span>Mijn donateurschap opzeggen</span></label></div>{memberAction === "INCREASE" ? <label>Gewenst nieuw maandbedrag (€)<input inputMode="decimal" min="1" name="requestedAmount" required step="0.01" type="number" /></label> : null}{memberAction === "CANCEL" ? <p className="survey-note">Uw donateurschap wordt niet direct stopgezet. Een beheerder controleert eerst uw verzoek.</p> : null}<button className="donor-submit-button survey-submit" disabled={pending} type="submit">{pending ? "Verzoek opslaan..." : "Keuze bevestigen"}</button></form>;
  ...
  return <form action={preview ? undefined : action} className="survey-card">
    ...
    {isOneTime ? <fieldset className="survey-section"><legend>Donatie voor {donationSubject(survey.title)}</legend><p className="survey-note">Waar is de donatie voor bedoeld? <strong>{donationSubject(survey.title)}</strong></p><div className="survey-grid"><label>{settings.oneTimeAmountLabel}<input inputMode="decimal" max="100000" min="1" name="oneTimeAmount" placeholder={settings.oneTimeAmountPlaceholder} required step="0.01" type="number" /></label>{!anonymousDonation ? <label>Uw naam (optioneel als u anoniem kiest)<input autoComplete="name" maxLength={120} name="fullName" required /></label> : null}</div><label className="survey-consent"><input checked={anonymousDonation} className="w-auto" name="anonymousDonation" onChange={(event) => setAnonymousDonation(event.target.checked)} type="checkbox" /><span>Ik wil anoniem doneren</span></label>{error("fullName")}{error("oneTimeAmount")}</fieldset>
    : <>
      <fieldset className="survey-section"><legend><span>1</span> {settings.question1}</legend><div className="survey-choices"><Choice label={settings.yesLabel} name="isExistingDonor" onChange={() => { setExisting("yes"); setJoin(null); }} value="yes" /><Choice label={settings.noLabel} name="isExistingDonor" onChange={() => { setExisting("no"); setJoin(null); }} value="no" /></div>{existing === "yes" ? <p className="survey-note">{settings.existingDonorNote}</p> : null}{error("isExistingDonor")}</fieldset>
      {existing === "no" ? <fieldset className="survey-section survey-reveal"><legend><span>2</span> {settings.question2}</legend><div className="survey-choices"><Choice label={settings.joinYesLabel} name="wantsToBecomeDonor" onChange={() => setJoin("yes")} value="yes" /><Choice label={settings.joinNoLabel} name="wantsToBecomeDonor" onChange={() => setJoin("no")} value="no" /></div>{error("wantsToBecomeDonor")}{join === "yes" ? <div className="survey-mandate survey-reveal"><label>{settings.monthlyAmountLabel}<input inputMode="decimal" min="1" name="monthlyAmount" placeholder="Bijvoorbeeld 10,00" required step="0.01" type="number" /></label>{error("monthlyAmount")}<label className="survey-consent"><input name="directDebitConsent" required type="checkbox" /><span>{settings.consentText}</span></label>{error("directDebitConsent")}</div> : join === "no" ? <p className="survey-note">{settings.noMembershipNote}</p> : null}</fieldset> : null}
    </>}
    {state.message ? <p className="survey-error" role="alert">{state.message}</p> : null}
    {(canSubmit || preview) ? <button className="donor-submit-button survey-submit" disabled={pending || preview} type={preview ? "button" : "submit"}>{preview ? "Voorbeeld – verzenden uitgeschakeld" : pending ? (isOneTime ? "Donatie opslaan..." : "Antwoorden verzenden...") : isOneTime ? "Donatie opslaan (test)" : settings.submitLabel}</button> : null}
