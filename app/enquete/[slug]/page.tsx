...
import { defaultSepaConfig, getSepaConfig } from "@/lib/monthly-donation-agreement";
...
export default async function PublicSurveyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sepaConfig = survey.templateKey === "ONE_TIME_DONATION" ? defaultSepaConfig : await getSepaConfig();
  return <main className="survey-page"><SurveyForm sepaConfig={sepaConfig} settings={parseFixedSurveySettings(survey.questions)} survey={survey} /></main>;
}
