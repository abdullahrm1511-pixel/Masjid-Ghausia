...
export default async function PublicSurveyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const survey = await prisma.survey.findUnique({ where: { slug }, select: { id: true, title: true, description: true, templateKey: true, isActive: true, startsAt: true, endsAt: true } });
  if (!survey) notFound();
