...
export function mosqueDonationSummary(items: PaymentLike[]) {
  ...
  const correctionItems = donationItems.filter((item) => item.status === "PAID" && item.amountCents < 0);
  ...
  const openTotalCents = dueItems.reduce((sum, item) => sum + item.amountCents, 0);
  const paidTotalCents = paidItems.reduce((sum, item) => sum + item.amountCents, 0);
  const correctionTotalCents = Math.abs(correctionItems.reduce((sum, item) => sum + item.amountCents, 0));
  const settledTotalCents = paidTotalCents + correctionTotalCents;
  const dueTotalCents = Math.max(openTotalCents - settledTotalCents, 0);
